...
import { CreatePerumahanType, UpdatePerumahanType } from "../../models/perumahan-model";
...
export const DeleteUnitPerumahanRepo = async <T extends DataResponse>(id: number) => {
  const res: HttpResponse<T> = await deleteDataWithToken(`/unit/${id}`);

  return res;
};

export const CreateUnitPerumahanRepo = async <T extends DataResponse>(body: CreateUnitRequest) => {
...
export const UpdatePerumahanRepo = async <T extends DataResponse>(
  ...
  const res: HttpResponse<T> = await patchDataWithToken(`/perumahan/${id}`, body);

  return res;
};

export const DeletePerumahanRepo = async <T extends DataResponse>(id: number) => {
  const res: HttpResponse<T> = await deleteDataWithToken(`/perumahan/${id}`);

  return res;
};

export const CreatePerumahanRepo = async <T extends DataResponse>(body: CreatePerumahanType) => {
