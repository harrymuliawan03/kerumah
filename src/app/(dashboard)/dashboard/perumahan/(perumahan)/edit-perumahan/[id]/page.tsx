...
import { GetPerumahanByIdCase, GetUnitByIdCase, UpdatePerumahanCase, UpdateUnitCase } from "@/modules/perumahan/usecases/perumahan/perumahan.usecase";
...
import { PerumahanResponse, UpdatePerumahanType } from "@/modules/perumahan/models/perumahan-model";
import { PerumahanUpdateSchema } from "@/modules/perumahan/models/schema";

/* eslint-disable react/no-unescaped-entities */
const EditPerumahanPage = ({ params }: { params: { id: number } }) => {
  const router = useRouter();

  const [perumahan, setPerumahan] = useState<PerumahanResponse>();
  ...
  const res = await GetPerumahanByIdCase(params.id);
    ...
    if (res.success) {
      ...
      setPerumahan(undefined);
    }
  };

  const onSubmit = async (data: UpdatePerumahanType) => {

    const res = await UpdatePerumahanCase(params.id, data)
  ...
  const {
    ...
    getValues,
    formState: { errors }
  } = useForm<UpdatePerumahanType>({
    mode: 'all',
    resolver: yupResolver(PerumahanUpdateSchema)
  })
        ...
        <div className="flex flex-wrap -mx-3 mb-2">
          ...
          <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
            <Controller
                name="kode_unit"
                control={control}
                defaultValue={perumahan?.kode_unit}
                render={({ field }) => (
                  <InputComponent
                    title="Kode Unit"
                    defaultValue={field.value ?? undefined}
          ...
          <div className="flex flex-wrap -mx-3 mb-2">
            <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                ...
                name="jml_unit"
                control={control}
                defaultValue={perumahan?.jml_unit}
                render={({ field }) => (
                  <InputComponent
                    title="Jumlah Unit"
                    defaultValue={field.value ?? undefined}
            ...
            <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
              <Controller
                ...
                control={control}
                defaultValue={perumahan?.periode_pembayaran ?? "year"}
                render={({ field }) => (
                  <SelectComponent
                    data={periode}
                    selected={field.value ?? ''}
                    title="Periode Pembayaran"
                    defaultValue={perumahan?.periode_pembayaran ?? "Belum diisi"}
                    onChange={(e) => {
                      const newValue = e.target.value;
                      field.onChange(newValue);
                    }}
            ...
            <div className="flex flex-wrap -mx-3 mb-2">
              <div className="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                <Controller
                  name="provinsi"
                  control={control}
                  defaultValue={perumahan?.provinsi ?? ""}
                  render={({ field }) => (
                    <SelectComponent
              ...
              <div className="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                <Controller
                  name="kode_pos"
                  control={control}
                  defaultValue={perumahan?.kode_pos ?? undefined}
                  render={({ field }) => (
                    <InputComponent
                      title="Kode Pos"
                      defaultValue={field.value ?? undefined}
