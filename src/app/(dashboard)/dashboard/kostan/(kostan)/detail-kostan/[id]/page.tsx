...
export default function DetailKostanPage({
  params,
}: {
  params: { id: number };
}) {
  ...
  const getUnits = async () => {
    const res = await GetUnitsKostanCase(params.id);
  ...
  const handleDelete = async () => {
    if (params?.id) {
      const res = await DeleteKostanCase(params?.id);
      if (res.success) {
        ...
        {units?.map((item, index) => {
          return (
            <CardPropertiDetail
              key={index}
              onClick={() => {
                router.push(`/dashboard/kostan/detail-unit/${item.id}`);
              }}
              type="Kostan"
              data={item}
            />
